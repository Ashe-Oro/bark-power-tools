...
function buildDetailedBarkPowerOutput(barkPowerData, hbarkBalance, accountId, userData) {
    ...
    output += buildMoreDetailsSection(barkPowerData, hbarkBalance, accountId, userData);

    return output;
}

// Helper function to build the "More Details" section
function buildMoreDetailsSection(barkPowerData, hbarkBalance, accountId, userData) {
...
function updateUIElementsForDetailedView(barkPowerData) {
    const barkPowerUsed = barkPowerData.todayAllocatedBarks - barkPowerData.barkingPower;
    const barkPowerPercentageUsed = (barkPowerUsed / barkPowerData.todayAllocatedBarks) * 100;

    toggleElementDisplay("toggleDetails", "block");
    toggleElementDisplay("extraDetails", "none");
    toggleElementDisplay("progressContainer", "block");
    toggleElementDisplay("clearSearch", "block");
    toggleElementDisplay("output", "block");

    updateProgressBar(barkPowerPercentageUsed);
}

// Helper function to build output for unlinked users who have received barks
function buildUnlinkedUserBarksReceivedOutput(barkPowerData) {
...
function buildBasicInfoOutput(hbarkBalance, accountId, userData) {
    let output = '';
...
function toggleUIElementsForBasicView(accountId, userData) {
    if (accountId || (userData && userData.twitterHandle)) {
        toggleElementDisplay("toggleDetails", "block");
        toggleElementDisplay("extraDetails", "none");
    }
...
async function fetchBarksRemaining() {
    ...
    try {
        ...
        data.forEach((item) => {
            addUserCell(item, leaderboardTable);
        });
...
function addUserCell(item, leaderboardTable) {
